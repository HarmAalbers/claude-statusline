...
if git -C "$DIR" rev-parse --git-dir > /dev/null 2>&1; then
    ...
    if [ -n "$REPO_PATH" ]; then
        ...
        if [ -n "$TARGET_BRANCH" ]; then
            ...
            # Display link text without OSC 8 ANSI hyperlinks
            # Note: OSC 8 escape sequences with BEL (\a) cause Claude Code to exit
            GITHUB_LINK=" [${LINK_TEXT}]"
        fi
    fi
...
if [ -n "$VIM_MODE" ]; then
    case "$VIM_MODE" in
        NORMAL)  VIM_INDICATOR=" \033[1;34m[N]\033[0m" ;;  # Blue
        INSERT)  VIM_INDICATOR=" \033[1;32m[I]\033[0m" ;;  # Green
        *)       VIM_INDICATOR=" \033[0;90m[${VIM_MODE:0:1}]\033[0m" ;;  # Dim for unknown
    esac
fi
...
if [ -n "$WORKTREE_NAME" ]; then
    WORKTREE_INDICATOR="\033[1;33m🌳${WORKTREE_NAME}\033[0m "
fi

LINE2="${WORKTREE_INDICATOR}🌿 \033[1;36m${BRANCH}\033[0m${GITHUB_LINK}${GIT_STATUS}${SIZE_LABEL}"
